...
namespace TokenGeneratorTests
{
    ...
    public class AuthorizeControllerTest
    {
        ...
        public AuthorizeControllerTest()
        {
            ...
            var tenantTableHelper = new Mock<TenantTableHelper>();
            tenantTableHelper.Setup(t => t.GetUserTenantInfo(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>())).Returns(Task.FromResult(new TenantModel { PartitionKey = userId, RowKey= iotTenantId, Roles="[]", Timestamp = DateTime.Now }));


            this.ac = new AuthorizeController(this._config) { keyVaultHelper = keyVaultHelper.Object , tenantTableHelper = tenantTableHelper.Object};
        }
