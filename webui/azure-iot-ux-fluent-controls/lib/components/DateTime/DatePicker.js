...
class DatePicker extends React.Component {
    constructor(props) {
        ...
        this.onBlur = (e) => {
            if (e.relatedTarget && !this._containerRef.contains(e.relatedTarget)) {
                this.setState({ expanded: false });
            }
        };
    ...
    parse(newValue) {
        ...
        if (valid) {
            let parsed = new Common_1.MethodDate(this.props.localTimezone, year, month - 1, date);
    ...
    render() {
        ...
        return (React.createElement(Attributes_1.Elements.div, { methodRef: this.setContainerRef, className: css('date-picker-container', this.props.className), attr: this.props.attr.container, onBlur: this.onBlur },
            React.createElement(Attributes_1.Elements.div, { className: css('date-picker-input-container'), attr: this.props.attr.inputContainer },
                React.createElement(Attributes_1.Elements.input, { type: 'text', name: this.props.name, value: this.state.value, className: inputClassName, placeholder: placeholder, onChange: this.onChange, onPaste: this.onPaste, required: this.props.required, disabled: this.props.disabled, methodRef: this.inputRef, attr: this.props.attr.input }),
                React.createElement(ActionTrigger_1.ActionTriggerButton, { icon: 'calendar', className: css('date-picker-calendar-icon'), onClick: this.onExpand, disabled: this.props.disabled, attr: this.props.attr.inputIcon, "aria-haspopup": true, "aria-expanded": this.state.expanded })),
            this.state.expanded &&
                React.createElement(Attributes_1.Elements.div, { className: css('date-picker-dropdown', {
                        'above': this.props.showAbove
                    }) },
                    React.createElement(Calendar_1.Calendar, { value: value, onChange: this.onSelect, className: css('date-picker-calendar'), year: parsed.year || null, month: parsed.month - 1, localTimezone: this.props.localTimezone, locale: this.props.locale, attr: this.props.attr.calendar }),
                    React.createElement("div", { className: css('date-picker-dropdown-triangle') }))));
    }
}
