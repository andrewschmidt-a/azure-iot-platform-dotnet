...
const StyledButton = styled_components_1.default(Attributes_1.Elements.button) `
    &&&&& {
        ...
        &:disabled {
            ...
            background-color: ${(props) => props.theme.colorBgBtnStandardDisabled};
        }
        >*:hover {
            background-color: ${(props) => props.theme.colorBgBtnStandardHover};
        }
...
exports.ActionTriggerButton = (props) => {
    ...
    return (React.createElement(StyledButton, { type: 'button', onClick: onClick, className: css('action-trigger-button', {
            'disabled': disabled
        }, className), disabled: disabled, tabIndex: tabIndex, title: label, attr: attr && attr.button },
        React.createElement(ActionTrigger_1.ActionTrigger, { icon: icon, rightIcon: rightIcon, label: label, disabled: disabled, attr: attr })));
};
