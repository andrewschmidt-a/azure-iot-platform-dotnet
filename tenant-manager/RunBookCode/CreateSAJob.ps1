...
$eventHubNamespaceName = $data.eventHubNamespaceName # need to check with Joe
$eventHubAccessPolicyKey = $data.eventHubAccessPolicyKey
$iotHubName = $data.iotHubName
$iotHubConnectionString = $data.iotHubConnectionString
...
function addConsumerGroup($iotHubName){
    Add-AzIotHubEHCG -ResourceGroupName $resourceGroup `
                     -Name $iotHubName `
                     -EventHubEndpointName "events" `
                     -EventHubConsumerGroupName "streamingJobsConsumerGroup"

    Write-Host "Consumer Group added to the Iothub $iotHubName"
}
...
# Add EeventHub outout to SA Job
...
# update the flag IsAlertingEnabled to true in table storage
function updateUserTenantTtable($tenantId){
    ...
    $row.IsAlertingEnabled = $true
    $row | Update-AzTableRow -Table $table
    Write-Output "Flag IsAlertingEnabled updated for tenant $tenantId"
}
